...
if ($method === 'GET') {
    ...
    $data = array_map(
        function ($row) use ($imagesByListingId, $commentsByListingId, $likesByListingId, $likedByUser) {
            error_log('LISTING JSON => ' . json_encode($row));
            return map_listing_row(
                $row,
                $imagesByListingId,
                $commentsByListingId,
                $likesByListingId,
...
";
    ...
    $stmt = $pdo->prepare("
        SELECT id, user_id AS user_id, type, status, title, description, city, location_text, is_boosted, created_at
        FROM listings
        WHERE id = :id
    ");
    $stmt->execute([':id' => $id]);
